...
import { mapToRenderLocation } from '../../utils/helpers'
...
export class RenderSystem extends System {
  ...
  update(_deltaTime: number): void {
    ...
    for (const renderable of renderables) {
      ...
      if (movement && movement.isMoving && movement.targetLocation) {
        const offset = this.calculateMovementOffset(movement)
        offsetX = offset.x
        offsetY = offset.y
      }
  ...
  private calculateMovementOffset(movement: Movement): Location {
